...
class WxHongBaoHelper
{
	...
	*/
zlog($this->parameters);
		if($this->parameters["mchid"] == null || 
	...
	/**
	  例如：
		...
		9CF3B7"
	 */
	protected function get_sign($apikey){
       try {
			...
			return $md5SignUtil->sign($unSignParaString,$commonUtil->trimString($apikey));
	...
	*/
	function create_hongbao_xml($apikey){
		try 
		{
		    $this->setParameter('sign', $this->get_sign($apikey));
		    $commonUtil = new CommonUtil();
	...
	function curl_post_ssl($url, $vars, $second=30,$aHeader=array())
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT,EXTEND_PATH.'weixin/hongbao/cert/apiclient_cert.pem');
 		curl_setopt($ch,CURLOPT_SSLKEY,EXTEND_PATH.'weixin/hongbao/cert/apiclient_key.pem');
 		curl_setopt($ch,CURLOPT_CAINFO,EXTEND_PATH.'weixin/hongbao/cert/rootca.pem');
